...
class RAGEngine:
    ...
    def __init__(self):
        try:
            ...
            logger.error(f"Failed to load sentence transformers: {e}. Falling back to basic keywords.")
            self.model = None
            
        # Dictionary to store conversation history {conversation_id: [{"role": "user", "parts": ...}]}
        self.conversations = {}
    ...
    async def query(self, question: str, conversation_id: str = None) -> dict:
        ...
        try:
            import requests
            api_key = "AIzaSyCyMZnPq07ehRLbfbOobBWk0PBxESn-tkg"
